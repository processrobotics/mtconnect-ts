...
const _radians = new THREE.Vector3(
  (1.0 / 180.0) * Math.PI,
  (1.0 / 180.0) * Math.PI,
  (1.0 / 180.0) * Math.PI,
)
...
class CoordinateSystem {
  constructor(doc, comp) {
    ...
    if (this.Transformation) {
      ...
      if (this.Transformation.Translation) {
        const trans = new THREE.Vector3().fromArray(
  ...
  shape() {
    if (!this.obj3d) {
      ...
      this.obj3d = new THREE.LineSegments(
        edges,
  ...
  resolve() {
    ...
    if (this.coordinateSystemIdRef) {
      this.coordinateSystem =
        this.component.entities[this.coordinateSystemIdRef]
    }
  }
}
  ...
  initTransform() {
    if (this.Transformation) {
      ...
      const line = new THREE.LineSegments(
        edges,
        new THREE.LineBasicMaterial({ color: "rgba(255, 0, 0, 1)" }),
      )
      this.model.add(line)
    }
  }
...
class Fixture {
  ...
  apply(obs) {
    ...
    const solidmodel = new Object()
    Object.assign(solidmodel, obs)
    if (solidmodel.Scale) {
      solidmodel.Scale = solidmodel.Scale.split(/[ ]+/).map((s) =>
        Number.parseFloat(s),
      )
    }
    if (solidmodel.attach) {
      solidmodel.parent = this.dataItem.component.root.findGeometry(
        solidmodel.attach,
      )
    }
    ...
    if (solidmodel.Translation || solidmodel.Rotation) {
      solidmodel.Transformation = {
        Translation: solidmodel.Translation
          ? solidmodel.Translation.split(/[ ]+/).map((s) =>
              Number.parseFloat(s),
            )
          : null,
        Rotation: solidmodel.Rotation
          ? solidmodel.Rotation.split(/[ ]+/).map((s) => Number.parseFloat(s))
          : null,
      }
    }
...
class DataItem {
  ...
  apply(key, data) {
    if (this.category === "CONDITION") {
      ...
      if (
        this.machine &&
        (this.type === "POSITION" || this.type === "ANGLE") &&
        this.subType === "ACTUAL" &&
        typeof value === "number"
      ) {
        const child = this.component.attachChild
        if (child?.geometry && this.axis) {
          const vec = this.axis.clone().multiplyScalar(value)
          if (
            this.type === "POSITION" &&
            this.component.motion.coordinateSystem
          ) {
            const comp = this.component.motion.coordinateSystem.translation
              .clone()
              .negate()
              .multiply(this.axis)
            vec.add(comp)
...
class Component {
  ...
  parse(doc, parser) {
    ...
    if (this.Configuration) {
      ...
      if (config.CoordinateSystems) {
        this.coordinateSystems = parser.coordinateSystems(
          config.CoordinateSystems,
          (c) => new CoordinateSystem(c, this),
        )
        console.log(this.coordinateSystems)
      }
    }
    if (this.DataItems) {
      this.dataItems = parser.dataItems(this, (d) => new DataItem(d, this))
    }
  }
  ...
  attach() {
    if (
      ...
      if (
        ...
        this.motion.type === "REVOLUTE" &&
        (this.motion.translation.x !== 0 ||
          this.motion.translation.y !== 0 ||
          this.motion.translation.z !== 0)
      ) {
        const mod = this.attachChild.geometry.model
        const geo = mod.geometry

        const v = new THREE.Vector3(
          this.motion.translation.x / 1000.0,
  ...
  findDataItems(pred) {
    ...
    return items.concat(this.children.flatMap((c) => c.findDataItems(pred)))
  }
  ...
  findComponents(type, name = undefined) {
    ...
    return comps.concat(
      this.children.flatMap((c) => c.findComponents(type, name)),
    )
  }
