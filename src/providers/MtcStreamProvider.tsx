...
const DEFAULT_AGENT_ADDRESS = import.meta.env.VITE_MTC_AGENT_URL || 'localhost:5000';

export const MtcStreamProvider: React.FC<MtcStreamProviderProps> = ({ children, agentAddress }) => {
  ...
  const value: MtcStreamContextValue = useMemo(() => ({ ...stream }), [
  ...
  return <MtcStreamContext.Provider value={value}>{children}</MtcStreamContext.Provider>;
};

export function useMtcStreamContext(): MtcStreamContextValue {
  const ctx = useContext(MtcStreamContext);
  if (!ctx) {
    throw new Error("useMtcStreamContext must be used within an MtcStreamProvider");
  }
  return ctx;
}
